...
| 指令              | 功能描述                                        | 範例                             |
|-------------------|-------------------------------------------------|----------------------------------|
| `G90`             | 切換為絕對座標模式（僅 X/Y/Z，預設）           | `G90`                           |
| `G91`             | 切換為相對座標模式（僅 X/Y/Z）                 | `G91`                           |
| `M82`             | 設定擠出機為絕對模式                           | `M82`                       |
| `M83`             | 設定擠出機為相對模式                           | `M83`                       |
...
> 模式相容性（對齊 Marlin）：`G90/G91` 只影響 `X/Y/Z`；`M82/M83` 只影響 `E`。
...
G1 X10 E2 F1200
G91
G1 X5 E4 F1200   ; E 在 M82 下仍為絕對，最終 E=4（不是 +4）
M83
G1 X5 E1 F1200   ; E 在 M83 下為相對，最終 E=5
```
...
| 座標顯示模式   | `X100 Y100` / `Z10 E500` 或 `ΔX10 ΔY0` 等 |
...
- 預設 `extrusionTotalMM = -1`（未設定時不顯示進度，可用 `M290` 設定總量）
...
完成 `G28` 歸零後，噴頭與成像平台之間通常仍會有少許間隙，可依照下列兩種方式設定
首層高度：

1. **切片軟體調整 Z offset**：於軟體端輸入微量負值，確保首層能緊貼平台。
2. **手動校正**：將噴頭移動至貼近平台後，依序輸入
