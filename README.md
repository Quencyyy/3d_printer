...
若要使用音樂(蜂鳴器)功能，需將 `BUZZER_PIN` 腳位接出連接蜂鳴器，否則預設不會發聲。
...
在 `main.ino` 取消註解 `#define ENABLE_AXIS_CYCLE_TEST` 後，韌體會進入專用的軸循環測試介面。LCD 與序列埠僅顯示此模式的訊息，不再提供一般選單或 G-code 解析。

按下一次按鈕可開始或停止目前軸的往返移動，連按兩次則會依序切換至下一軸（X → Y → Z → E）。此模式可用來確認馬達接線與運動方向是否正確。
